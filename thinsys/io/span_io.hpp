...
template <>
size_t thinsys::io::input_traits<std::span<const std::byte>>::read(
...
template <>
size_t thinsys::io::input_traits<std::span<std::byte>>::read(
    std::span<std::byte>& in, std::span<std::byte> dest,
    std::error_code&) noexcept {
  const auto sz = std::min(in.size(), dest.size());
  std::ranges::copy(in.subspan(0, sz), dest.data());
  in = in.subspan(sz);
...
template <>
size_t thinsys::io::output_traits<std::span<std::byte>>::write(
    std::span<std::byte>& out, std::span<const std::byte> buf,
    std::error_code&) noexcept {
